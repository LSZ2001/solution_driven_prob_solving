...
jsPsych.plugins['survey-multi-select'] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    trial_form.addEventListener('submit', function(event) {
      ...
      for(var i=0; i<trial.questions.length; i++){
        var match = display_element.querySelector('#jspsych-survey-multi-select-'+i);
        var idx_chosen = [];
        ...
        for(var j=0; j<inputboxes.length; j++){
          ...
          idx_chosen.push(currentChecked.getAttribute('idx'))
        }
      }
      if (idx_chosen.length != 2) {
        return false
      }
      ...
      var trial_data = {
        ...
        choice1: idx_chosen[0],
        choice2: idx_chosen[1],
